...
def count_oranges(img, kernel_size):
    total_area = get_total_circle_surface(img)
    area_one_element = np.pi * (kernel_size/2)**2
...
orange_grey = cv2.resize(orange_grey, (math.floor(orange_grey.shape[1]/2), math.floor(orange_grey.shape[0]/2)))
tree_grey = cv2.imread("iivp/pictures/orangetree.jpg", 0)
tree_grey = cv2.resize(tree_grey, (math.floor(tree_grey.shape[1]/4), math.floor(tree_grey.shape[0]/4)))
